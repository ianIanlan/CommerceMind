...
本文档说明 CommerceMind 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

重要说明：CommerceMind 当前已经支持可直接调用的端到端评测能力。启动 HTTP 服务后，可以通过 Swagger 或 `curl` 调用 `POST /eval/run`，自动评测意图识别准确率、端到端 Agent 回复质量、LLM-as-Judge 四维评分、回归检测和优化建议。该能力不是文档设计稿，而是由 `evaluation/evaluator.py` 和 `api/main.py` 中的 `/eval/run` 接口真实实现。

CommerceMind 是一个企业级智能客服系统，核心链路为：
...
```env
COMMERCEMIND_SKILLS_DIR=./skills
COMMERCEMIND_SKILLS_MAX_PROMPT_CHARS=5000
```

### 2.3 全栈部署和 run 开发模式的区别

CommerceMind 常用两种 Docker 启动方式：`docker compose up` 全栈部署，以及 `docker run` 开发模式。两者最大的区别是：**全栈部署会同时启动应用和依赖服务；run 开发模式通常只手动运行一个应用容器，依赖服务需要提前启动**。

| 对比项 | Docker Compose 全栈部署 | Docker run 开发模式 |
|--------|--------------------------|----------------------|
| 启动命令 | `docker compose up -d --build` | `docker run ... commercemind ...` |
| 启动内容 | CommerceMind、Redis、ChromaDB、Prometheus、Nginx | 只启动你指定的单个容器 |
| Redis/ChromaDB | 自动启动并加入同一网络 | 必须先执行 `docker compose up -d redis chromadb` |
| 容器网络 | Compose 自动创建并管理 | 需要手动指定 `--network commercemind_commercemind-network` |
...
- 如果只是跑 CLI，最省心的方式是 `docker compose run --rm commercemind python api/main.py --cli`，它会自动使用 Compose 网络。
...
看到 CommerceMind 启动日志并且健康检查通过后，服务可用。

启动后的端口：

| 服务 | 容器名 | 宿主机端口 | 容器内端口 | 用途 |
|------|--------|------------|------------|------|
| CommerceMind API | `commercemind-app` | `8000` | `8000` | 主 API 服务 |
| Nginx | `commercemind-nginx` | `80` | `80` | 反向代理 |
| ChromaDB | `commercemind-chromadb` | `8001` | `8000` | 向量数据库 |
| Redis | `commercemind-redis` | `6379` | `6379` | 工作记忆 |
| Prometheus | `commercemind-prometheus` | `9090` | `9090` | 监控数据 |
...
```bash
docker compose build --no-cache commercemind
```
...
```bash
docker run -it --rm \
  --network commercemind_commercemind-network \
  -p 8000:8000 \
  -e ANTHROPIC_BASE_URL="https://api.deepseek.com/anthropic" \
  -e ANTHROPIC_API_KEY="your_key" \
  -e ANTHROPIC_MODEL="deepseek-v4-pro" \
  -e REDIS_URL="redis://:commercemind123@redis:6379/0" \
...
CommerceMind 基于 FastAPI 构建，启动 HTTP 服务后可以直接在浏览器访问 Swagger UI 调用接口。
...
这类问题会先计算 `general`、`technical`、`billing` 的领域分数，最高分成为 `primary_agent`；随后通过明确的意图和领域关键词检测复合问题，将命中的其他专业 Agent 放入 `supporting_agents`。如果识别到多个领域，系统会并行执行主 Agent 和辅助 Agent，并在回复中标注“主处理/辅助处理”。
...
CommerceMind 的知识库由 `mcp/knowledge_base.py` 管理，底层使用 ChromaDB collection：
...
CommerceMind 使用了三个 ChromaDB collection：
...
```bash
docker exec -it commercemind-chromadb curl http://localhost:8000/api/v1/heartbeat
```
...
```bash
docker exec -it commercemind-app bash
```
...
docker volume inspect commercemind_chromadb-data
```

查看容器内数据目录：

```bash
docker exec -it commercemind-chromadb sh
...
```bash
docker compose down
docker volume rm commercemind_chromadb-data
docker compose up -d --build
```
...
```bash
docker exec -it commercemind-redis redis-cli -a commercemind123
```
...
```bash
docker compose restart commercemind
```
...
```bash
docker exec -it commercemind-redis redis-cli -a commercemind123 ping
```
...
2. 等待几秒
3. 查看 `docker compose logs -f commercemind` 是否出现 `用户画像已更新`
...
# 6. 检索
curl -X POST "http://localhost:8000/search?query=CommerceMind如何接入API&top_k=3"
