...
class MCPToolManager:
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None, model: str = "claude-3-5-sonnet-20241022"):
        ...
        self._client = build_llm_client(api_key=api_key, base_url=base_url)
        self._model  = model
...
返回 JSON 数组，例如: ["子查询1", "子查询2", "子查询3"]"""
    ...
    async def search_with_rewrite(
        ...
        """
        # 1. 查询改写：生成多角度子查询
        rewrite_enabled = os.getenv("RAG_QUERY_REWRITE_ENABLED", "true").lower() in {
            "1", "true", "yes", "on",
        }
        sub_queries = await self.rewrite_query(query, n=3) if rewrite_enabled else [query]
        ...
        # 3. 合并去重。政策版本 + chunk 是稳定身份；旧数据才回退到内容哈希。
        seen, merged = set(), []
        for r in results:
            if isinstance(r, ToolResult) and r.success and isinstance(r.data, list):
                for item in r.data:
                    if isinstance(item, dict):
                        key = item.get("document_id") or ":".join(str(item.get(k, "")) for k in ("policy_id", "version", "chunk"))
                        if key == "::":
