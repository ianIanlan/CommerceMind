"""
RAG 知识库 —— 基于 ChromaDB 的真实检索实现。

功能：
  1. 文档导入：将文本切片、生成离线 Embedding 后存入 ChromaDB
  2. 语义检索：根据 query 从知识库中检索最相关的文档片段
  3. 与 MCP 工具框架集成：作为 knowledge_search 工具的真实 handler

ChromaDB 在这里的角色：
  - memory/ 中用于存储对话记忆（情景记忆 + 用户画像）
  - 这里用于存储知识库文档（RAG 检索）
  两者是不同的 collection，互不干扰。
"""
import asyncio
import hashlib
import logging
from datetime import datetime, timezone
from typing import Any, Dict, List, Optional

from core.embeddings import local_text_embedding, local_text_embeddings

logger = logging.getLogger(__name__)


class KnowledgeBase:
    """
    基于 ChromaDB 的 RAG 知识库。

    默认使用无需下载模型的字符 n-gram 向量，并把向量显式交给 ChromaDB。
    因此容器首次启动不依赖外部模型站点；生产环境也可以替换为训练型
    Embedding 服务，而无需改变 ChromaDB 的存储与检索接口。
    """

    COLLECTION_NAME = "knowledge_base"

    def __init__(
        self,
        chroma_host: str = "localhost",
        chroma_port: int = 8000,
        chroma_path: str = "./data/chroma",
    ):
        # 延迟导入：政策元数据校验等纯逻辑不应强制依赖 ChromaDB。
        import chromadb

        # 优先连接独立 ChromaDB 服务（服务端内置 embedding 模型，客户端无需下载）
        self._use_server = False
        try:
            # HttpClient 默认也会初始化 ChromaDB telemetry；显式关闭避免 posthog 兼容性错误日志。
            self._client = chromadb.HttpClient(
                host=chroma_host,
                port=chroma_port,
                settings=chromadb.Settings(anonymized_telemetry=False),
            )
            self._client.heartbeat()
            self._use_server = True
            logger.info(f"知识库 ChromaDB 已连接: {chroma_host}:{chroma_port}")
        except Exception:
            logger.info(f"知识库 ChromaDB 服务不可用，使用本地模式: {chroma_path}")
            self._client = chromadb.PersistentClient(
                path=chroma_path,
                settings=chromadb.Settings(anonymized_telemetry=False),
            )

        self._collection = self._client.get_or_create_collection(
            name=self.COLLECTION_NAME,
            metadata={"description": "CommerceMind RAG 知识库"},
        )

        # 如果知识库为空，导入默认文档
        if self._collection.count() == 0:
            self._load_default_docs()

    # ── 文档管理 ──────────────────────────────────────────────────────────────

    def add_documents(self, documents: List[Dict[str, Any]]) -> int:
        """
        批量导入文档到知识库。

        documents 格式: [{"title": "...", "content": "..."}, ...]
        长文档会自动切片（每片 500 字）。
        """
        ids, docs, metas = [], [], []

        for doc in documents:
            title   = str(doc.get("title", ""))
            content = str(doc.get("content", ""))
            metadata = doc.get("metadata") if isinstance(doc.get("metadata"), dict) else {}
            base_meta = {
                "policy_id": str(metadata.get("policy_id") or hashlib.md5(title.encode()).hexdigest()[:12]),
                "version": str(metadata.get("version") or "1.0"),
                "status": str(metadata.get("status") or "active"),
                "effective_at": str(metadata.get("effective_at") or ""),
                "expires_at": str(metadata.get("expires_at") or ""),
                "category": str(metadata.get("category") or "general"),
                "source": str(metadata.get("source") or "internal"),
            }
            chunks  = self._chunk_text(content, chunk_size=500)

            for i, chunk in enumerate(chunks):
                doc_id = hashlib.md5(
                    f"{base_meta['policy_id']}:{base_meta['version']}:{i}:{chunk[:50]}".encode()
                ).hexdigest()
                ids.append(doc_id)
                docs.append(chunk)
                metas.append({
                    "title": title,
                    "chunk_index": i,
                    "total_chunks": len(chunks),
                    **base_meta,
                })

        if ids:
            self._collection.upsert(
                ids=ids,
                documents=docs,
                metadatas=metas,
                embeddings=local_text_embeddings(docs),
            )
            logger.info(f"知识库导入 {len(ids)} 个文档片段")

        return len(ids)

    async def add_documents_async(self, documents: List[Dict[str, Any]]) -> int:
        """异步导入文档；ChromaDB 客户端为同步实现，因此放入线程池执行。"""
        return await asyncio.to_thread(self.add_documents, documents)

    def search(self, query: str, top_k: int = 5, category: str = "") -> List[Dict[str, Any]]:
        """
        语义检索：根据 query 返回最相关的文档片段。

        ChromaDB 内部自动将 query 转为向量，与存储的文档向量做余弦相似度匹配。
        """
        candidate_k = min(max(top_k * 3, top_k), max(self._collection.count(), 1))
        query_args: Dict[str, Any] = {
            "query_embeddings": [local_text_embedding(query)],
            "n_results": candidate_k,
        }
        if category:
            query_args["where"] = {"category": {"$in": [category, "general"]}}
        results = self._collection.query(**query_args)

        items = []
        if results["documents"] and results["documents"][0]:
            for doc, meta, dist in zip(
                results["documents"][0],
                results["metadatas"][0],
                results["distances"][0],
            ):
                if not self._metadata_is_active(meta):
                    continue
                items.append({
                    "title":    meta.get("title", ""),
                    "content":  doc,
                    # 默认 Chroma 使用平方 L2 距离。向量已做单位归一化，
                    # 因此 cosine = 1 - squared_l2 / 2。
                    "score":    self._distance_to_similarity(dist),
                    "chunk":    meta.get("chunk_index", 0),
                    "policy_id": meta.get("policy_id", ""),
                    "version": meta.get("version", ""),
                    "category": meta.get("category", "general"),
                    "source": meta.get("source", "internal"),
                    "document_id": f"{meta.get('policy_id', '')}:{meta.get('version', '')}:{meta.get('chunk_index', 0)}",
                })
                if len(items) >= top_k:
                    break

        return items

    @staticmethod
    def _distance_to_similarity(distance: float) -> float:
        """Convert normalized-vector squared L2 distance to a 0..1 score."""
        return round(max(0.0, min(1.0, 1.0 - float(distance) / 2.0)), 4)

    @staticmethod
    def _metadata_is_active(metadata: Dict[str, Any], now: Optional[datetime] = None) -> bool:
        """过滤未生效、已失效或显式停用的政策。时间统一按 ISO-8601 解析。"""
        if str(metadata.get("status") or "active").lower() != "active":
            return False
        now = now or datetime.now(timezone.utc)

        def parse(value: Any) -> Optional[datetime]:
            text = str(value or "").strip()
            if not text:
                return None
            try:
                parsed = datetime.fromisoformat(text.replace("Z", "+00:00"))
                return parsed if parsed.tzinfo else parsed.replace(tzinfo=timezone.utc)
            except ValueError:
                logger.warning("忽略无法解析的政策时间: %r", value)
                return None

        effective_at = parse(metadata.get("effective_at"))
        expires_at = parse(metadata.get("expires_at"))
        return not ((effective_at and effective_at > now) or (expires_at and expires_at <= now))

    async def search_async(self, query: str, top_k: int = 5, category: str = "") -> List[Dict[str, Any]]:
        """异步检索；ChromaDB 客户端为同步实现，因此放入线程池执行。"""
        return await asyncio.to_thread(self.search, query, top_k, category)

    @property
    def doc_count(self) -> int:
        return self._collection.count()

    async def doc_count_async(self) -> int:
        """异步获取文档片段数量。"""
        return await asyncio.to_thread(self._collection.count)

    # ── MCP 工具 handler ─────────────────────────────────────────────────────

    async def search_handler(self, params: Dict[str, Any], context: Any) -> List[Dict]:
        """
        作为 MCP 工具的 handler 注册。

        MCPToolManager.register(Tool(
            name="knowledge_search",
            handler=kb.search_handler,
            ...
        ))
        """
        query = params.get("query", "")
        top_k = params.get("top_k", 5)
        category = str((context or {}).get("category", ""))
        return await self.search_async(query, top_k=top_k, category=category)

    # ── 内部方法 ──────────────────────────────────────────────────────────────

    def _chunk_text(self, text: str, chunk_size: int = 500) -> List[str]:
        """将长文本按 chunk_size 切片，保留语义完整性（按句号/换行切分）。"""
        if len(text) <= chunk_size:
            return [text] if text.strip() else []

        chunks = []
        current = ""
        # 按句子切分
        sentences = text.replace("\n", "。").split("。")
        for sent in sentences:
            sent = sent.strip()
            if not sent:
                continue
            if len(current) + len(sent) + 1 > chunk_size:
                if current:
                    chunks.append(current)
                current = sent
            else:
                current = f"{current}。{sent}" if current else sent

        if current:
            chunks.append(current)

        return chunks

    def _load_default_docs(self) -> None:
        """导入默认知识库文档（客服场景常见问题）。"""
        default_docs = [
            {
                "title": "退款政策",
                "metadata": {
                    "policy_id": "refund-general",
                    "version": "2.0",
                    "status": "active",
                    "effective_at": "2026-01-01T00:00:00+08:00",
                    "category": "after_sales",
                    "source": "commerce_policy_center",
                },
                "content": (
                    "退款政策说明。"
                    "符合条件的普通实物商品在签收后 7 天内可以申请无理由退货退款。"
                    "虚拟商品、已激活服务和特殊商品不适用普通七天无理由规则，需要按商品政策人工核验。"
                    "退款申请提交后，系统会在 1-3 个工作日内审核。"
                    "审核通过后，款项将在 5-7 个工作日内退回原支付账户。"
                    "如果商品已发货，需要先完成退货流程才能退款。"
                    "退货运费由用户承担，除非是商品质量问题。"
                    "超过 7 天但未超过 30 天的订单，需要提供商品质量问题的证据才能退款。"
                ),
            },
            {
                "title": "订单查询",
                "metadata": {"policy_id": "order-query", "version": "1.0", "category": "order", "source": "commerce_policy_center"},
                "content": (
                    "订单查询指南。"
                    "用户可以通过订单号查询订单状态。"
                    "订单状态包括：待支付、已支付、已发货、运输中、已签收、已完成。"
                    "如果订单显示已发货但超过 7 天未收到，可以联系客服申请查件。"
                    "物流信息通常在发货后 24 小时内更新。"
                    "如果订单显示异常，请提供订单号联系客服处理。"
                ),
            },
            {
                "title": "账户安全",
                "metadata": {"policy_id": "account-security", "version": "1.0", "category": "security", "source": "security_center"},
                "content": (
                    "账户安全说明。"
                    "建议用户定期修改密码，密码长度至少 8 位，包含字母和数字。"
                    "如果忘记密码，可以通过绑定的手机号或邮箱重置。"
                    "发现账户异常登录时，系统会自动锁定账户并发送通知。"
                    "用户可以在安全设置中开启两步验证，提高账户安全性。"
                    "不要将密码分享给他人，客服人员不会索要用户密码。"
                ),
            },
            {
                "title": "技术故障排查",
                "metadata": {"policy_id": "technical-guide", "version": "1.0", "category": "technical", "source": "support_center"},
                "content": (
                    "常见技术问题排查。"
                    "应用崩溃：请尝试清除缓存后重启应用，如果问题持续请更新到最新版本。"
                    "登录失败 401 错误：表示认证失败，请检查用户名密码是否正确，或尝试重置密码。"
                    "页面加载慢：检查网络连接，尝试切换 WiFi 或移动数据。"
                    "支付失败：确认银行卡余额充足，检查是否开启了网上支付功能。"
                    "500 服务器错误：这是服务端问题，请稍后重试，如果持续出现请联系技术支持。"
                ),
            },
            {
                "title": "会员与积分",
                "metadata": {"policy_id": "membership", "version": "1.0", "category": "membership", "source": "commerce_policy_center"},
                "content": (
                    "会员积分规则。"
                    "每消费 1 元累积 1 积分。"
                    "积分可以在下次购物时抵扣，100 积分 = 1 元。"
                    "会员等级分为：普通会员、银卡会员（累计消费 1000 元）、金卡会员（累计消费 5000 元）。"
                    "银卡会员享受 95 折优惠，金卡会员享受 9 折优惠。"
                    "积分有效期为 1 年，过期自动清零。"
                    "生日当月消费可获得双倍积分。"
                ),
            },
            {
                "title": "配送说明",
                "metadata": {"policy_id": "delivery", "version": "1.0", "category": "order", "source": "logistics_center"},
                "content": (
                    "配送服务说明。"
                    "标准配送：3-5 个工作日送达，免运费（订单满 99 元）。"
                    "加急配送：1-2 个工作日送达，运费 15 元。"
                    "同城配送：当日达或次日达，运费 10 元。"
                    "偏远地区可能需要额外 2-3 天。"
                    "配送时间为每天 9:00-18:00，节假日可能延迟。"
                    "如果需要修改收货地址，请在发货前联系客服。"
                ),
            },
        ]
        self.add_documents(default_docs)
        logger.info(f"已导入默认知识库: {len(default_docs)} 篇文档")
