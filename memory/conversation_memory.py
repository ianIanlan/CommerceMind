...
关键设计：
  ...
  - 默认使用稳定的本地字符 n-gram 向量，容器启动无需下载模型
"""
import hashlib
import asyncio
import json
...
from core.embeddings import local_text_embedding
...
返回格式: {{"preferences": ["..."], "entities": {{"产品": [], "问题类型": []}}}}"""
        ...
        try:
            ...
            await asyncio.to_thread(
                ...
                ids=[doc_id],
                documents=[doc_text],
                embeddings=[local_text_embedding(doc_text)],
                metadatas=[{
                    "user_id": user_id,
                    "conv_id": conv_id,
    ...
    async def _store_episodic(self, user_id: str, conv_id: str, text: str, summary: str) -> None:
        ...
        try:
            ...
            await asyncio.to_thread(
                ...
                ids=[doc_id],
                documents=[summary],
                embeddings=[local_text_embedding(summary)],
    ...
    async def _query_episodic(
        ...
        return await asyncio.to_thread(
            ...
            query_embeddings=[local_text_embedding(query_text)],
            n_results=n_results,
            where=where,
        )
